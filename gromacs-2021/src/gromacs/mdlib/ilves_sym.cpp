...
IlvesSym::IlvesSym(const t_commrec *const cr,
                   const InteractionDefinitions &idef,
                   const real *const invmass,
                   const int nthreads,
                   std::vector<std::map<std::string, std::chrono::duration<double, std::milli>>>
                       &durations)
    : Ilves(cr, idef, invmass, nthreads, true, durations) {}

std::pair<bool, int> IlvesSym::solve(const ArrayRef<const RVec> x,
    ...
    #pragma omp parallel num_threads(nthreads)
    {
...
#endif
        // General block A.
        make_lhs(tid, x_ab, x_ab);

        if (dep_par) {
            // Schur block A.
            #pragma omp master
            { make_lhs(nthreads, x_ab, x_ab); }
        ...
        durations[tid]["LDLT"] += end - start;
#endif

        // No need to wait. The scheduling will be the same in the next function
        // (sparrow_Cholesky_solve).
